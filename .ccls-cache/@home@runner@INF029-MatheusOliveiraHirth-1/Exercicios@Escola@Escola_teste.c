...
int main(void) {
  ...
  qtd_aluno = 0, excluir, sair, cont, cpf_invalido, j;
    ...
    case 1: {
      ...
      do {
        ...
        case 3: {
          ...
          for (i = 0; i <= MAX_ALUNO && aluno[i].existe == 1; i++) {
            if ((strcmp(aluno_auxiliar[i].nome, aluno_auxiliar[i + 1].nome) > 0) && aluno[i + 1].existe == 1) { // posso utilizar a struct antiga como referência, já
       // que se tratam de cópias
        ...
        case 5: {
            if(cadastrar_aluno(qtd_aluno, aluno) == 1){
            qtd_aluno++;
